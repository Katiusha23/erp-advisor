export function calcTCO(tcoData) {
  ...
  const bugetImpl  = parseFloat(tcoData.buget_implementare) || 0;
  ...
  const roi3ani = (bugetImpl > 0)
    ? Math.round(((economiiAnuale * 3 - bugetImpl) / bugetImpl) * 100)
    : null;

  return { costuriActuale, economiiAnuale, breakEvenLuni, roi3ani };
}
